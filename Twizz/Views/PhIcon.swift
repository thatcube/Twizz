...
///
/// Set `bold` to use Phosphor's heavier `bold` weight for small glyphs that
/// would otherwise read too thin (e.g. checkmarks). `bold` and `filled` are
/// distinct Phosphor weights, so only one may be applied at a time; `filled`
/// wins if both are set. Only icons whose `-bold` asset is bundled may set it.
struct PhIcon: View {
  let icon: Ph
  ...
  private var assetName: String {
    let suffix = filled ? "-fill" : (bold ? "-bold" : "")
    return "ph-\(icon.rawValue)\(suffix)"
  }
